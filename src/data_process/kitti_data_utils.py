...
def get_filtered_lidar(lidar, boxes3d=None, cls_ids=None):
    ...
    if boxes3d is not None:
        box_x = (boxes3d[:, :, 0] >= minX) & (boxes3d[:, :, 0] < maxX)
        box_y = (boxes3d[:, :, 1] >= minY) & (boxes3d[:, :, 1] < maxY)
        box_z = (boxes3d[:, :, 2] >= minZ) & (boxes3d[:, :, 2] < maxZ)
        mask_box = np.sum(box_x & box_y & box_z, axis=1) > 0
        boxes3d = boxes3d[mask_box]
        if cls_ids is not None:
            return lidar, boxes3d, cls_ids[mask_box]
        else:
